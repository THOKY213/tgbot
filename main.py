...
def back_kb(target="main"):
    return InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Главное меню", callback_data=f"back_{target}")]])

def back_and_action_kb(action_label, action_cb, back_target="main"):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(action_label, callback_data=action_cb)],
        [InlineKeyboardButton("◀️ Главное меню", callback_data=f"back_{back_target}")],
    ])
...
async def cmd_del(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if not ctx.args:
        await update.message.reply_text(
            "Использование: `/del <ID>`\n\nСписок клиентов: нажмите 📋 в меню /start",
            parse_mode=ParseMode.MARKDOWN
        )
        return
    try:
    ...
    if not client:
        await update.message.reply_text(f"❌ Клиент с ID `{cid}` не найден.", parse_mode=ParseMode.MARKDOWN)
        return
...
async def remind_pick(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if target == "r_all":
        ...
        count = len(ctx.user_data["remind_ids"])
        hint = f"для *{count} клиентов*"
    else:
...
def parse_rows_from_table(rows_iter):
    """Принимает итератор списков значений, возвращает (col_map, data_rows, detected_msg)."""
    rows = list(rows_iter)
    if not rows:
        return None, [], "Файл пуст."

    # Пробуем определить заголовки в первой строке
    col_map = {}
    ...
    if "name" in col_map and "amount" in col_map:
        ...
        # Заголовков нет — порядок по умолчанию
...
def import_rows(col_map, data_rows):
    """Записывает строки в БД, возвращает (imported, errors)."""
    imported, errors = 0, []
...
async def import_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await q.message.edit_text(
        ...
        "*Обязательные колонки* (любое из названий):\n"
        "• ФИО / Имя / Клиент / Должник / Контрагент / name / client...\n"
        "• Сумма / Долг / Задолженность / Итого / amount / debt...\n\n"
        "*Необязательные:*\n"
        "• Телефон / Контакт / phone...\n"
        "• Описание / Назначение / Договор / description...\n"
        "• Дата / Срок / Дата оплаты / due date...\n\n"
        "📎 *Отправьте файл:*\n\n/cancel — отмена",
        reply_markup=None,
        parse_mode=ParseMode.MARKDOWN
    )
...
async def import_file(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            f"❌ Для формата `{ext}` нужна библиотека: `pip install {lib}`",
            parse_mode=ParseMode.MARKDOWN
        )
        return WAIT_FILE
    ...
    if col_map is None or "name" not in col_map or "amount" not in col_map:
        await update.message.reply_text(
            ...
            "Назовите их одним из вариантов:\n"
            "• *Имя/ФИО/Клиент/Должник* и *Сумма/Долг/Задолженность*",
            parse_mode=ParseMode.MARKDOWN
        )
        return WAIT_FILE
...
async def cancel(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ctx.user_data.clear()
    await update.message.reply_text("❌ Отменено.", reply_markup=main_menu_kb(), parse_mode=ParseMode.MARKDOWN)
    return ConversationHandler.END
